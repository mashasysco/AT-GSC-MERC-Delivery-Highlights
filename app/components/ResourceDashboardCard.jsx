...
export default function ResourceDashboardCard() {
  ...
  const items = data['resource-dashboard'] || []
  const shouldHideAdmin = view === 'leadership'

  const handleAdd = () => {
    if (newLink.trim()) {
      addLink('resource-dashboard', newLink.trim())
      setNewLink('')
    }
  }
  ...
  const handleSaveEdit = (idx) => {
    if (editValue.trim()) {
      updateLink('resource-dashboard', idx, editValue.trim())
      setEditing(null)
      setEditValue('')
    }
  }
    ...
    <Card number="04" title="Review Resource Dashboard">
                    ...
                    {!shouldHideAdmin && (
                      <>
                        ...
                        <button
                          ...
                          onClick={() => removeLink('resource-dashboard', idx)}
                          style={{
                            background: 'none',
                            border: 'none',
      ...
      {!shouldHideAdmin && (
        <div className="add-item admin-only">
          <input
            type="text"
            placeholder="Paste resource dashboard link (URL)..."
            value={newLink}
